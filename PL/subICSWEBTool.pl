...
{ my $q=@_[0];
  ...
  #########################
  print hr();
  print $q->submit(-name=>'ShowInfo:SignIn', -value=>'Help', -id=>'helpButton');
...
#  &initializeAllPersonnelData;
  ...
  { my ($val,$label)=@_;
    ...
    my $str ="<tr>".
      ...
      "<td> <input type=submit name='ShowInfo:$val' value='help' id='helpButton'></td>".
      "</tr> ";
  } 
...
{ my ($q) = @_;
  ...
  { my @MultiPerson = map { @tmp=split(/\t/,$_); ($tmp[2] =~ /MultiPerson/) ? $tmp[0]:""; } @roleList;
    ...
    &SubmitActionList(@actions);
    print hr(),$q->submit(-name=>'ShowInfo:ICCStaffingAssignment', -value=>'Help', -id=>'helpButton');
  }
...
{ my @damageAddress=&DamageReportAddresses;	
  ...
  tie(%MapStreetAddresses,"DB_File","$ICSdir/DB/MapStreetAddresses.db",O_RDWR,0666);
...
{ my ($q)=@_;
  ...
  &SubmitActionList('Cancel>Home');
  print hr(),$q->submit(-name=>'ShowInfo:HowToReviews', -value=>'Help', -id=>'helpButton');
  print $q->end_form;
}
